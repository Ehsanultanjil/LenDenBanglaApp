...
import { View, ScrollView, Switch, Alert, Image } from 'react-native';
import { Bell, Shield, CircleHelp, LogOut, ChevronRight, Languages } from 'lucide-react-native';
...
export default function ProfileScreen() {
  ...
  // Shares the same React Query cache as the other tabs, so these counts stay
  // in sync with what the user actually has.
    ...
    <ScrollView className="flex-1 bg-bg">
      ...
      <View className="px-5 pb-8">
        <View className="bg-card rounded-3xl p-5 items-center">
          {avatarUrl ? (
            <Image source={{ uri: avatarUrl }} className="w-20 h-20 rounded-full mb-4" />
          ) : (
        ...
        <View className="mt-6 bg-card rounded-3xl overflow-hidden">
          <SettingsRow
            icon={<Bell size={18} color="#46D6A8" />}
